...
export const isValidBlockedSite = (site: unknown): site is BlockedSite => {
  ...
  if (typeof s.allowedMinutesPerHour !== 'number' || s.allowedMinutesPerHour < 1 || s.allowedMinutesPerHour > 60)
    return false;
  ...
  if (
    ...
    !schedule.workDays.every((d: unknown) => typeof d === 'number' && d >= 0 && d <= 6)
  )
    return false;
...
export const isValidSettings = (
  data: unknown,
): { valid: boolean; errors: string[]; normalized: FocusSettings | null } => {
  const errors: string[] = [];
